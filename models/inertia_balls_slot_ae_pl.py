...
from slot_based_disentanglement.utils.disentanglement_utils import linear_disentanglement, permutation_disentanglement
...
#         return F.mse_loss(reconstructions, images, reduction="sum")
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        # linear regression
        reg = LinearRegression().fit(obj_slots.reshape(-1, slot_dim).clone().cpu(), z_disentanglement.reshape(-1, z_dim).clone().cpu())
        obj_projected =  torch.tensor(reg.predict(obj_slots.reshape(-1, slot_dim).clone().cpu()), device=obj_slots.device) # [dataset_size * n_balls, z_dim]
        h_z_disentanglement = obj_projected.reshape(-1, n_balls * z_dim) # [dataset_size, n_balls * z_dim]
