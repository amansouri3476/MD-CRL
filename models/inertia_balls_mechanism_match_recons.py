...
import slot_based_disentanglement
from slot_based_disentanglement.utils.additional_loggers import get_img_rec_table_data
from slot_based_disentanglement.models.contrastive_pl import Contrastive
from slot_based_disentanglement.utils.general import init_weights
from slot_based_disentanglement.utils.lp_solver import lp_solver_pulp, lp_solver_cvxpy
from slot_based_disentanglement.utils.segmentation_metrics import adjusted_rand_index
from slot_based_disentanglement.utils.graphic_utils import draw_scene
import wandb
import hydra
...
class Inert(Contrastive):
    ...
    def common_step(self, batch, batch_idx, stage="train"):
        ...
        if self.hparams["use_all_balls_mcc"]:
            # 1. remove all background slots from the candidates and send the rest n_balls slot
            # projections to be compared with true z. In cases where the background slot contains
            # an object this will be problematic. Below you can find this method.
            # TODO: only the background slot should be discarded and the rest of the slots (when 
            # n_slots=n_balls+1) projections should be used for mcc computation. However, in some
            # cases the background slot contains an object, for now we should avoid those cases as much
            # as possible.
